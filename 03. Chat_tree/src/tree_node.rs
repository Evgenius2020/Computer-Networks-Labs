...
impl TreeNode {
    ...
    pub fn broadcast(&mut self, message: &[u8], filter_addr: Option<SocketAddr>) {
    ...
    fn send(&self, buf: &[u8], addr: &SocketAddr) -> bool {
        self.socket.send_to(buf, addr).expect("send_to error");
